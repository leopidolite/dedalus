...
def EVP_solve(k_x, target_omega, alph, f0):
    ...
    eigs  = solver.eigenvalues[order] # Eigenmodes in order
...
# %%
# Run EVP for multiple target ω values. 
    # Lists of parameters corresponding to each target frequency for setting different initial conditions

f0s= [1.0]
omtargets= [-0.7]
alphas = [10] # alpha parameters for double tanh equator
...
ani.save(path +f'/{datetime.now().strftime("%y-%m-%d %H:%M")}_tanh_yanai_h_kx{kx}_alpha{alpha}.mp4', writer=writer)
plt.show()

# Save data in .npz file
np.savez(path + f'/alpha{alpha}kx{kx:.3f}data.npz', h_list = h_list, u_list = u_list)
